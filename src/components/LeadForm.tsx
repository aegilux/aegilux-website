...
const FALLBACK_MAILTO = "hello@aegilux.com";
...
const LeadForm = ({ className = "", showPhoneLink = false }: LeadFormProps) => {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (FORM_ENDPOINT) {
        ...
        // Fallback: open mailto: with prefilled body so the form still functions.
        const body = encodeURIComponent(
          `Name: ${form.name}\nEmail: ${form.email}\nPhone: ${form.phone}\nCompany: ${form.companyName}\nAWS accounts: ${form.awsAccounts}\n\nMessage:\n${form.message}\n\n--\nUTM: ${JSON.stringify(utm)}\nReferrer: ${referrer}`,
        );
        const subject = encodeURIComponent("Aegilux — Snapshot Enquiry");
    ...
    return (
      <div className={`glass p-8 rounded-lg text-left neon-border ${className}`}>
        ...
        <div className="mt-6 flex flex-col sm:flex-row gap-3">
          <a
            href="mailto:hello@aegilux.com?subject=Aegilux%20Snapshot%20Enquiry"
